...
#from UsedCar.items import PostItem
#import UsedCar.items
import UsedCar.settings
...
class UsedCarSpider(scrapy.Spider):
     ...
     def parse(self, response):  
          if len(response.xpath('//div[@class="over18-notice"]')) > 0:
               if self._retries < UsedCarSpider.MAX_RETRY:
                    self._retries += 1
               ...
               if self._pages < UsedCarSpider.MAX_PAGES:
